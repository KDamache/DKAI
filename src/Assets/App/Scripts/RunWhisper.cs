...
public class RunWhisper : MonoBehaviour
{
    ...
    /// <summary>
    /// Transcrit un AudioClip (mono 16k recommandé). Renvoie le texte.
    /// </summary>
    public async Task<string> TranscribeClipAsync(AudioClip clip, SystemLanguage language = SystemLanguage.English)
    {
        InitializeIfNeeded();

        // Prépare les tokens de départ
        outputString = "";
        tokenCount = 0;

        int langToken = language switch
        {
        ...
        audioClip = clip;
        LoadAudio();
        EncodeAudio();           // utilise spectrogram + encoder (appelés dans EncodeAudio)
        transcribe = true;

        // Boucle d’inférence (même logique que l’ancien Start)
        ...
        // Nettoyage des tenseurs transitoires (on garde les workers pour réutiliser)
        audioInput.Dispose();
    ...
    void LoadAudio()
    {
        int clipSamples = Mathf.Min(audioClip.samples, maxSamples);
        var temp = new float[clipSamples];
        audioClip.GetData(temp, 0);

        // Pad à 30s (Whisper attend un tenseur 30s * 16k)
        var data = new float[maxSamples];
        System.Array.Copy(temp, data, clipSamples);
    ...
    async Awaitable InferenceStep()
    {
        ...
        past_key_values_0_decoder_key.Dispose(); past_key_values_0_decoder_value.Dispose(); past_key_values_1_decoder_key.Dispose(); past_key_values_1_decoder_value.Dispose();
        past_key_values_2_decoder_key.Dispose(); past_key_values_2_decoder_value.Dispose(); past_key_values_3_decoder_key.Dispose(); past_key_values_3_decoder_value.Dispose();
        past_key_values_0_encoder_key.Dispose(); past_key_values_0_encoder_value.Dispose(); past_key_values_1_encoder_key.Dispose(); past_key_values_1_encoder_value.Dispose();
        past_key_values_2_encoder_key.Dispose(); past_key_values_2_encoder_value.Dispose(); past_key_values_3_encoder_key.Dispose(); past_key_values_3_encoder_value.Dispose();
